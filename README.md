...
- 💿 **A/B 双面**：A 面收录选曲，B 面呈现按 10 秒间隔规则识别的跳过记录、独立短评和侧写；点击翻面切换明暗配色，支持减少动态效果
- 📤 **分享手记**：复制本期直达链接、保存 A/B 斜切双面 1080 × 1350 封面图片，公开手记附本期二维码；支持时可调用系统分享。图片在浏览器生成，不调用模型；私人手记和本地示例仅分享图片
